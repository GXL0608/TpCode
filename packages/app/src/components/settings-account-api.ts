...
export async function parseAccountError(response?: Response) {
  ...
  if (code === "solution_missing") return "解决方案不存在"
  if (code === "solution_exists") return "同一产品下的解决方案编码已存在"
  if (code === "solution_binding_exists") return "当前产品已绑定该解决方案"
  if (code === "solution_binding_missing") return "当前产品与解决方案的绑定关系不存在"
  if (code === "solution_name_invalid") return "解决方案名称不能为空"
  if (code === "solution_code_invalid") return "解决方案编码不能为空"
  if (code === "solution_project_missing") return "解决方案默认项目不存在"
  if (code === "solution_build_profile_invalid") return "编译打包配置格式不正确"
  if (code === "solution_root_missing") return "请至少配置一个源码根目录"
  if (code === "solution_root_directory_invalid") return "源码根目录不能为空"
  if (code === "solution_root_directory_missing") return "源码根目录不存在或不可访问"
  if (code === "solution_root_virtual_members_missing") return "虚拟目录组至少需要一个成员目录"
  if (code === "build_job_missing") return "构建任务不存在"
  if (code === "build_job_running") return "构建任务正在执行中"
  if (code === "build_artifact_missing") return "构建产物不存在"
  if (code === "coding_no_changes") return "AI 未在沙盒内产生任何代码变更"
  if (code === "build_workdir_missing") return "构建目录配置无效，当前 workdir 在构建沙盒中不存在"
  if (code === "compile_command_unconfigured") return "当前解决方案仍在使用默认占位编译命令，请先配置真实 compile_command"
  if (code === "prompt_text_missing") return "请输入构建需求"
  if (code === "saved_plan_missing") return "计划不存在"
  ...
  if (code === "provider_not_configured") return "当前引用了未配置的供应商"
  if (code === "model_not_configured") return "当前引用了未配置供应商下不可用的模型"
