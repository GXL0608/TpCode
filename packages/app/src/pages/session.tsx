...
export default function Page() {
  ...
  createEffect(
    on(
      () => [sdk.directory, params.dir, params.id] as const,
      ([, dir, id]) => {
        if (!id) return
        void (async () => {
          await untrack(() => sync.session.sync(id))
          await untrack(() => sync.session.todo(id))
        })().catch((error) => {
  ...
  createEffect(() => {
    ...
    const wants = desktopFileTreeOpen() || (desktopReviewOpen() && activeTab() === "review")
    if (!wants) return
