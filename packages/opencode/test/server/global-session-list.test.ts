...
describe("Session.listGlobal", () => {
  test("lists sessions across projects with project metadata", async () => {
    ...
    const sessions: Awaited<ReturnType<typeof Session.listGlobal>> extends AsyncGenerator<infer T> ? T[] : never = []
    for await (const session of Session.listGlobal({ limit: 200 })) sessions.push(session)
  ...
  test("excludes archived sessions by default", async () => {
    ...
    const allSessions: Awaited<ReturnType<typeof Session.listGlobal>> extends AsyncGenerator<infer T> ? T[] : never = []
    for await (const session of Session.listGlobal({ limit: 200, archived: true })) allSessions.push(session)
  ...
  test("supports cursor pagination", async () => {
    ...
    const page: Awaited<ReturnType<typeof Session.listGlobal>> extends AsyncGenerator<infer T> ? T[] : never = []
    for await (const session of Session.listGlobal({ directory: tmp.path, limit: 1 })) page.push(session)
    expect(page.length).toBe(1)
    expect(page[0].id).toBe(second.id)

    const next: Awaited<ReturnType<typeof Session.listGlobal>> extends AsyncGenerator<infer T> ? T[] : never = []
    for await (const session of Session.listGlobal({ directory: tmp.path, limit: 10, cursor: page[0].time.updated })) {
      next.push(session)
    }
